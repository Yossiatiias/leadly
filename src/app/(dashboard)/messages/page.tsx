      ...
      <div style={{ width: '260px', flexShrink: 0, background: 'var(--bg-surface)', borderLeft: '1px solid var(--border-default)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 16px 12px', borderBottom: '1px solid var(--border-default)' }}>
          ...
          <div style={{ position: 'relative' }}>
            <Search size={13} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--fg-4)' }} />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש..."
              style={{ width: '100%', padding: '7px 30px 7px 10px', border: '1px solid var(--border-default)', borderRadius: '8px', fontSize: '13px', background: 'var(--bg-sunken)', color: 'var(--fg-2)', outline: 'none', fontFamily: 'inherit' }} />
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filteredProfiles.map(p => {
            ...
            return (
              ...
              >
                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: isSelected ? '#3FA9DC' : 'var(--bg-hover)', color: isSelected ? 'white' : 'var(--fg-1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: '12px', flexShrink: 0 }}>
                  {getInitials(p.full_name)}
                </div>
                <div style={{ flex: 1, minWidth: 0, textAlign: 'right' }}>
                  <p style={{ fontWeight: 500, fontSize: '13px', color: 'var(--fg-2)' }}>{p.full_name}</p>
                  <p style={{ fontSize: '11px', color: 'var(--fg-4)' }}>{p.role === 'admin' ? 'מנהל' : 'נציג מכירות'}</p>
                </div>
                {unread > 0 && (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {!selectedId ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', color: 'var(--fg-4)' }}>
            ...
            <p style={{ fontSize: '14px', fontWeight: 500, color: 'var(--fg-3)' }}>בחר נציג להתחלת שיחה</p>
          </div>
        ) : (
          <>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-default)', background: 'var(--bg-surface)', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <p style={{ fontWeight: 500, fontSize: '14px', color: 'var(--fg-2)' }}>{selectedProfile?.full_name}</p>
                <p style={{ fontSize: '11px', color: 'var(--fg-4)' }}>{selectedProfile?.role === 'admin' ? 'מנהל' : 'נציג מכירות'}</p>
              </div>
            </div>

            <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {messages.length === 0 && (
                <p style={{ textAlign: 'center', color: 'var(--fg-4)', fontSize: '13px', marginTop: '40px' }}>אין הודעות עדיין</p>
              )}
              {messages.map((msg, i) => {
                  ...
                  <div key={msg.id}>
                    {showTime && (
                      <div style={{ textAlign: 'center', margin: '4px 0 8px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--fg-4)', background: 'var(--bg-hover)', padding: '2px 10px', borderRadius: '99px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: isMine ? 'flex-start' : 'flex-end' }}>
                      <div style={{
                        ...
                        background: isMine ? '#3FA9DC' : 'var(--bg-surface)',
                        color: isMine ? 'white' : 'var(--fg-2)',
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border-default)', background: 'var(--bg-surface)', display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
              <textarea ref={inputRef} value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKeyDown}
                ...
                rows={1}
                style={{ flex: 1, padding: '9px 13px', border: '1px solid var(--border-default)', borderRadius: '12px', fontSize: '14px', fontFamily: 'inherit', background: 'var(--bg-sunken)', color: 'var(--fg-2)', outline: 'none', resize: 'none', lineHeight: 1.5, maxHeight: '120px', overflowY: 'auto' }}
                onFocus={e => { e.target.style.borderColor = '#3FA9DC'; e.target.style.background = 'var(--bg-surface)' }}
              ...
              <button onClick={sendMessage} disabled={!text.trim() || sending}
                style={{ width: '40px', height: '40px', borderRadius: '50%', background: text.trim() ? '#3FA9DC' : 'var(--border-default)', border: 'none', cursor: text.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 140ms' }}>
                <Send size={15} style={{ color: text.trim() ? 'white' : 'var(--fg-4)', transform: 'scaleX(-1)' }} />
              </button>
            </div>
          </>
        )}
